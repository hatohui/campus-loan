...
/// Derives the visible device list from the catalogue, category chip, search
/// query and sort.
///
/// This is a pure derivation: it never mutates the source list from
/// [catalogueProvider]. Category and search are applied first (search is a
/// case-insensitive name filter over the already-loaded devices, so typing
/// never re-hits the network). Sorting always uses [Device.apiIndex] as a
/// stable tiebreaker, which is how the original API order is preserved, and
/// devices with an unknown price sort by their derived deposit ($20 tier) so
/// missing prices are handled consistently (CR#4).
